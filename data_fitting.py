...
def error_analyzer(x, Y, F):
    ...
    print(f'We found a least squares error of {leastsquerror(Y,F)[-1]} K')
    print(f'We found an absolute deviation error of {absdev_error(Y,F)[-1]} K')
    print(f'We found a Chebyshev error of {cheb_error(Y,F)} K')
